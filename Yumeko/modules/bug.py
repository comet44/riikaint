...
from Yumeko import pbot as Rikka
from Yumeko import (
    OWNER_ID as sneha_uwu_owo,
    OWNER_USERNAME as sneha,
    SUPPORT_CHAT as log,
)
...
@Rikka.on_message(filters.command("bug"))
@capture_err
...
**Event Stamp : ** **{datetimes}**"""
    ...
    if user_id == sneha_uwu_owo:
        if bugs:
            await msg.reply_text(
                "❎ <b>How can be Sneha reporting bug??</b>",
            )
            return
        else:
            await msg.reply_text(
                "Wah, Sneha You can't use this!"
            )
    elif user_id != sneha_uwu_owo:
        if bugs:
            await msg.reply_text(
...
@Rikka.on_callback_query(filters.regex("close_reply"))
async def close_reply(msg, CallbackQuery):
    await CallbackQuery.message.delete()

@Rikka.on_callback_query(filters.regex("close_send_photo"))
async def close_send_photo(_, CallbackQuery):
    is_Admin = await Rikka.get_chat_member(
